...
#ax3 = unioned.plot(kind="bar", x="club_name", y="overall", xlabel="", figsize=(15,5))
...
# ax1.hist(indices_vowel, freq1, alpha=0.5, label='x')
# ax1.hist(indices_consonants, freq2, alpha=0.5, label='y')
...
for letter in char1:
	ax1.bar(char1, freq1, alpha = 0.75, color = 'b')
	ax1.set_xticks(indices_vowel)
	ax1.set_xticklabels(char1)

for letter in char2:
	ax2.bar(char2, freq2, alpha = 0.75, color = 'r')
	ax2.set_xticks(indices_consonants)
	ax2.set_xticklabels(char2)
...
ax[2].bar(indices2, frequency2, alpha = 1, color = 'g')
...
ax3.set_xticks(indices2)
ax3.set_xticklabels(typeofchar)
...
plt.setp(ax[0], xlabel='Vowels')
plt.setp(ax[0], ylabel='Frequency')
plt.setp(ax[1], xlabel='Consonants')
plt.setp(ax[1], ylabel='Frequency')
plt.setp(ax[2], xlabel='Vowels vs Consonants')
plt.setp(ax[2], ylabel='Frequency')
...
plt.savefig("C:\\Users\\T.C\\Desktop\\Python Scripts\\CODI\\FreeCodeCamp\\Romeo\\Letters_Romeo.png")
